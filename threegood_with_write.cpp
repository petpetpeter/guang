...
double find_max(double*** mat,int nx,int ny,int nz){ // Initializing max element as INT_MIN                                        
    double maxElement = 0.;
    for (int i = 0; i <= nx-1; i++) {
        for (int j = 0; j <= ny-1; j++) {
	  for (int k = 0; k <= nz-1; k++){
...
void write_result(double*** gen, int nx, int ny, int nz, int t){
  //var = simulated data (array)
  //nx, ny = size (2d)
  //ts = timestep
  ofstream myfile;
  string file_name = "Test/3Dsim_" + to_string(t) + ".vtk";
  ...
  myfile << "DIMENSIONS " << nx-1 << " " << ny << " " << nz-2 << endl;
  myfile << "POINTS " << (nx-1)*ny*(nz-2) << " float\n";
