...
void setbound(double**u_new, double**v_new, double**w_new, double** p_new, int nx, int ny, int nz, double u_in){
...
void comp_FG(double**u, double**v, double**w, double**F, double**G, double**H, double dx, double dy, double donor, double Re, double gx, double gy, double ts, int nx, int ny){
  ...
  for ( int i = 1; i <= nx-3; i++){
	...
	F[i][j] = u[i][j] + ts*((ddu_dxdx+ddu_dydy+ddu_dzdz)/Re - duu_dx - duv_dy - duw_dz + gx);
      }
    }
  }
  ...
  for ( int j = 1; j <= ny-3; j++){
	...
	double dvv_dy = ( ( pow(((v[i][j][k]+v[i][j+1][k])/2),2) - pow(((v[i][j-1][k]+v[i][j][k])/2),2) ) / dy )  + (donor/dy)*(\abs(v[i][j][k]+v[i][j+1][k])*(v[i][j][k]-v[i][j+1][k])/4 - abs(v[i][j-1][k]+v[i][j][k])*(v[i][j-1][k]-v[i][j][k])/4 ) ;
        ...
        G[i][j] = v[i][j] + ts*((ddv_dxdx+ddv_dydy+ddv_dzdz)/Re - dvv_dy - duv_dx - dvw_dz + gy);
      }
    }
  }
  ...
  for ( int k = 1; k <= nz-3; k++){
	...
	double dww_dz = ( ( pow(((w[i][j][k]+v[i][j][k+1])/2),2) - pow(((w[i][j][k-1]+w[i][j][k])/2),2) ) / dz )  + (donor/dz)*(\abs(w[i][j][k]+w[i][j][k+1])*(w[i][j][k]-w[i][j][k+1])/4 - abs(w[i][j][k-1]+w[i][j][k])*(w[i][j][k-1]-w[i][j][k])/4 ) ;
        ...
        H[i][j] = w[i][j] + ts*((ddw_dxdx+ddw_dydy+ddw_dzdz)/Re - duw_dx - dvw_dy - dww_dz + gz);
      }
    }
  }
...
void comp_p( double**p, double**p_new, double**p_rhs, double dx, double dy, double dz,  int nx, int ny, int nz, int it_max, double eps){
  ...
  while ( it <= it_max && abs(r) >= eps){
    ...
    */ 
    ...
    for(int i = 1; i <= ny-2; i++){
      for(int j = 1; j <= nx-2; j++){
	for(int k = 1; j <= nz-2; k++){
	  res[i][j][k] = ((p_new[i+1][j][k]-p_new[i][j][k]) - (p_new[i][j][k]-p_new[i-1][j][k]))/(dx*dx)
	    +  ((p_new[i][j+1][k]-p_new[i][j][k]) - (p_new[i][j][k]-p_new[i-1][j][k]))/(dy*dy) + (p_new[i][j][k+1]-2*p_new[i][j][k]+p_new[i][j][k-1])/pow(dz,2)
                  - p_rhs[i][j];
	}
      }
    }
