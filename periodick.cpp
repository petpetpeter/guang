...
void setbound(double**u, double**u_new, double**v, double**v_new, double** p,double** p_new, int nx, int ny, double u_in){
  for (int i = 1; i <= ny-2; i++){
    u_new[i][0] = u_new[i][nx-3]; //period-stereamwise-east
    u_new[i][nx-2] = u_new[i][1]; //west
    v_new[i][0] = v_new[i][nx-3];// inflow for west bound
    v_new[i][1] = v_new[i][nx-2];// outflow for east bound
    v_new[i][nx-1] = v_new[i][2];
  }
  ...
  for ( int i=ny-2; i>=1; i--){
      p_new[i][0] = p[i][1]; //dp/dx == 0
      p_new[i][nx-1] = p[i][nx-2]; //p[i][nx-2]; // presure strip for west and east bound p=0
      p_new[i][1] = p[i][nx-2];
      //p_new[i][nx-1] = p_new[i][2];
    }
...
double find_max(double** mat,int nx,int ny){ // Initializing max element as INT_MIN                                        
    double maxElement = 0.;
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
...
void comp_p( double**p, double**p_new, double**p_rhs, double dx, double dy,  int nx, int ny, int it_max, double eps){
  ...
  while ( it <= it_max && abs(r) >= eps){
    ...
    for ( int i = ny-2; i>=1; i--){
      for ( int j = 1; j<=nx-2; j++){
	p_new[i][j] = (-0.7)*p[i][j] + (1.7/( 2/pow(dx,2) + 2/pow(dy,2) ))*( (p[i][j+1]+p_new[i][j-1])/pow(dx,2) 
                    + (p[i-1][j]+p_new[i+1][j])/pow(dy,2) - p_rhs[i][j]);
      }
    }
