...
void setbound(double**u, double**u_new, double**v, double**v_new, int nx, int ny, double u_in){
...
void comp_p( double**p, double**p_new, double**p_rhs, double dx, double dy,  int nx, int ny, int it_max, double eps){
  ...
  while ( it <= it_max && r >= eps){
    ...
    for ( int i=0; i<=ny-1; i++){
      p[i][0] = p[i][1];
      p[i][nx-1] = p[i][ny-2]; // presure strip for west and east bound
    }
    for ( int j=0; j<=nx-1; j++){
      p[0][j] = p[1][j];
      p[ny-1][j] = p[ny-2][j]; // pressure strip for north and south bound 
    }
    ...
    for ( int i=0; i<=ny-1; i++){
      p_new[i][0] = p_new[i][1];
      p_new[i][nx-1] = p_new[i][ny-2]; // presure strip for west and east bound
    }
    for ( int j=0; j<=nx-1; j++){
      p_new[0][j] = p_new[1][j];
      p_new[ny-1][j] = p_new[ny-2][j]; // pressure strip for north and south bound 
    }
}
...
void visualization( double**u_new, double**v_new, double**p_new, int nx, int ny, int t){
  cout << "t= " << t<< "\n";
  cout << "u_new" << "\n";
  for ( int i = 0; i<=ny-1; i++){
    for ( int j = 0; j<=nx-1; j++){
...
int main(){
  ...
  double gy = 0;
  int t_max = 10;
  int it_max = 10000000000; //for poisson iteration
  ...
  setbound(u, u_new, v, v_new, nx, ny, u_in);
  ...
  for ( int t = 0; t <= t_max-1; t++){
    ...
    visualization(u_new,v_new, p_new, nx, ny,t);
  }
}
